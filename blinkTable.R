  ...
  for(i in 1:length(dataASC)){
    ...
    for(j in 1:ntrials){
      temp<- data.frame(sub=NA, item=NA, cond=NA, blink_number = NA,
                        blink_start = NA, blink_end = NA, blink_pos = NA, blink_dur=NA)
      ...
      blinkflag <- trialDis[which(grepl('EBLINK',trialDis))]
      blinkstamp <- as.data.frame(do.call( rbind, strsplit(blinkflag, '\t' )))
      blinkstamp$V1 <- get_num(blinkstamp$V1)
      if(length(sblinkflag) != 0){
      for(k in 1:length(blinkstamp)){
        temp$blink_number[i] <- i
        temp$blink_start[i] <- blinkstamp$V1[i]
        temp$blink_end[i] <- blinkstamp$V2[i]
        temp$blink_dur[i] <- blinkstamp$V3[i]
        
        poscap <- trialDis[as.numeric(sblinkflag[i])-1]
        poscap <- as.data.frame(do.call( rbind, strsplit(poscap, '\t ' )))
        temp$blink_pos <- as.numeric(poscap$V2)
