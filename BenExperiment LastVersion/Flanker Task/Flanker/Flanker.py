...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/zeynepgunesozkan/Desktop/Flanker/Flanker.py',
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        if polygon.status == STARTED:
            ...
            if tThisFlipGlobal > polygon.tStartRefresh + random() + 1-frameTolerance:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    arrow_1.setImage(arrow_1)
    arrow_2.setImage(arrow_2)
    arrow_3.setImage(arrow_3)
    target.setImage(target)
    arrow_5.setImage(arrow_5)
    arrow_6.setImage(arrow_6)
    arrow_7.setImage(arrow_7)
    key_resp.keys = []
    key_resp.rt = []
    _key_resp_allKeys = []
    # keep track of which components have finished
    trialComponents = [arrow_1, arrow_2, arrow_3, target, arrow_5, arrow_6, arrow_7, key_resp]
    ...
    while continueRoutine:
        ...
        # *arrow_1* updates
        if arrow_1.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *arrow_7* updates
        if arrow_7.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            arrow_7.frameNStart = frameN  # exact frame index
            arrow_7.tStart = t  # local t and not account for scr refresh
            arrow_7.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(arrow_7, 'tStartRefresh')  # time at next scr refresh
            arrow_7.setAutoDraw(True)
    ...
    for thisComponent in trialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials.addData('arrow_1.started', arrow_1.tStartRefresh)
    trials.addData('arrow_1.stopped', arrow_1.tStopRefresh)
    trials.addData('arrow_2.started', arrow_2.tStartRefresh)
    trials.addData('arrow_2.stopped', arrow_2.tStopRefresh)
    trials.addData('arrow_3.started', arrow_3.tStartRefresh)
    trials.addData('arrow_3.stopped', arrow_3.tStopRefresh)
    trials.addData('target.started', target.tStartRefresh)
    trials.addData('target.stopped', target.tStopRefresh)
    trials.addData('arrow_5.started', arrow_5.tStartRefresh)
    trials.addData('arrow_5.stopped', arrow_5.tStopRefresh)
    trials.addData('arrow_6.started', arrow_6.tStartRefresh)
    trials.addData('arrow_6.stopped', arrow_6.tStopRefresh)
    trials.addData('arrow_7.started', arrow_7.tStartRefresh)
    trials.addData('arrow_7.stopped', arrow_7.tStopRefresh)
    # check responses
